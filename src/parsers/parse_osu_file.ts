...
import { beatmap_countdown, beatmap_data, 
    ...
    beatmap_data_hit_sound, beatmap_data_hit_sample, slider_properties, slider_type } from "../consts/beatmap_data";
                ...
                if (row_splitted.length >= 5) {
					...
					if (object_params && object_params.length > 0) {
						...
						if (options.is_parse_sliders && type == beatmap_data_hit_object_type.slider) {
							...
							slider_properties = {
								type: curve_data[0] as slider_type,
	...
	if (options.is_hit_objects_only_count === false && (is_properties_has_hit_objects_block || is_properties_has_timing_points_block) ){
		if (hit_objects.length > 1) {
			for ( let i = 0; i < hit_objects.length - 1; i++ ) {
				...
				if (hit_objects[i].type === beatmap_data_hit_object_type.hitcircle) {
					circles_time += time;
					circles_count++;
				} else if (hit_objects[i].type === beatmap_data_hit_object_type.slider) {
					sliders_time += time;
					sliders_count++;
				}
			}
		...
		if (properties_has_timing_points_block || osu_file_beatmap_properties.includes(osu_file_beatmap_property.circles_time)) {
			beatmap.general.circles_time = circles_time;
		}

		if (properties_has_timing_points_block || osu_file_beatmap_properties.includes(osu_file_beatmap_property.sliders_time)) {
			beatmap.general.sliders_time = sliders_time;
		}

		if (properties_has_timing_points_block || osu_file_beatmap_properties.includes(osu_file_beatmap_property.circles_count)) {
			beatmap.hit_objects.circles_count = circles_count;
		}
		if (properties_has_timing_points_block || osu_file_beatmap_properties.includes(osu_file_beatmap_property.sliders_count)) {

...
import { beatmap_property } from '../consts/parse_settings';
...
export class osu_db extends osu_file {
    ...
    constructor(file_path: string, parse_settings?: beatmap_property[] ){
    ...
    public osu_db_parse(): osu_db_results {
        ...
        if (this.parse_settings.indexOf(beatmap_property.beatmap_md5) == -1) {
    ...
    private beatmap_parse(osu_db_version: number): beatmap_results {
        ...
        if (osu_db_version < 20191106) {
            if (this.parse_settings.indexOf(beatmap_property.beatmap_size) != -1) {
        ...
        if (this.parse_settings.indexOf(beatmap_property.artist) != -1) {
            beatmap.artist = this.buff.getString();
        } else {
            this.buff.skipString();
        }
        if (this.parse_settings.indexOf(beatmap_property.artist_unicode) != -1) {
            beatmap.artist_unicode = this.buff.getString();
        } else {
            this.buff.skipString();
        }
        if (this.parse_settings.indexOf(beatmap_property.title) != -1) {
            beatmap.title = this.buff.getString();
        } else {
            this.buff.skipString();
        }
        if (this.parse_settings.indexOf(beatmap_property.title_unicode) != -1) {
            beatmap.title_unicode = this.buff.getString();
        } else {
            this.buff.skipString();
        }
        if (this.parse_settings.indexOf(beatmap_property.creator) != -1) {
            beatmap.creator = this.buff.getString();
        } else {
            this.buff.skipString();
        }
        if (this.parse_settings.indexOf(beatmap_property.difficulty) != -1) {
            beatmap.difficulty = this.buff.getString();
        } else {
            this.buff.skipString();
        }
        if (this.parse_settings.indexOf(beatmap_property.audio_filename) != -1) {
        ...
        if (this.parse_settings.indexOf(beatmap_property.osu_filename) != -1) {
            beatmap.osu_filename = this.buff.getString();
        } else {
            this.buff.skipString();
        }

        if (this.parse_settings.indexOf(beatmap_property.ranked_status) != -1) {
        ...
        if (this.parse_settings.indexOf(beatmap_property.number_hitcircles) != -1) {
            beatmap.number_hitcircles = this.buff.getShort();
        } else {
            this.buff.skipShort();
        }
        if (this.parse_settings.indexOf(beatmap_property.number_sliders) != -1) {
            beatmap.number_sliders = this.buff.getShort();
        } else {
            this.buff.skipShort();
        }
        if (this.parse_settings.indexOf(beatmap_property.number_spinners) != -1) {
            beatmap.number_spinners = this.buff.getShort();
        } else {
            this.buff.skipShort();
        }

        if (this.parse_settings.indexOf(beatmap_property.mod_date) != -1) {
        ...
        if (osu_db_version < 20140609) {
            if (this.parse_settings.indexOf(beatmap_property.beatmap_stats) != -1) {
        ...
        if (this.parse_settings.indexOf(beatmap_property.slider_velocity) != -1) {
        ...
        if (osu_db_version >= 20140609) {
            if (this.parse_settings.indexOf(beatmap_property.star_rating_std) != -1) {
                beatmap.star_rating_std = this.buff.getStarRatings();
            } else {
                this.buff.skipIntDoublePairs();
            }
            if (this.parse_settings.indexOf(beatmap_property.star_rating_taiko) != -1) {
                beatmap.star_rating_taiko = this.buff.getStarRatings();
            } else {
                this.buff.skipIntDoublePairs();
            }
            if (this.parse_settings.indexOf(beatmap_property.star_rating_ctb) != -1) {
                beatmap.star_rating_ctb = this.buff.getStarRatings();
            } else {
                this.buff.skipIntDoublePairs();
            }
            if (this.parse_settings.indexOf(beatmap_property.star_rating_mania) != -1) {
        ...
        if (this.parse_settings.indexOf(beatmap_property.drain_time) != -1) {
            beatmap.drain_time = this.buff.getInt();
        } else {
            this.buff.skipInt();
        }
        if (this.parse_settings.indexOf(beatmap_property.total_time) != -1) {
            beatmap.total_time = this.buff.getInt();
        } else {
            this.buff.skipInt();
        }
        if (this.parse_settings.indexOf(beatmap_property.preview_time) != -1) {
            beatmap.preview_time = this.buff.getInt();
        } else {
            this.buff.skipInt();
        }

        if (this.parse_settings.indexOf(beatmap_property.timing_points) != -1) {
            beatmap.timing_points = this.buff.getTimingPoints();
        } else {
            this.buff.skipTimingPoints();
        }

        if (this.parse_settings.indexOf(beatmap_property.beatmap_id) != -1) {
            beatmap.beatmap_id = this.buff.getInt();
        } else {
            this.buff.skipInt();
        }
        if (this.parse_settings.indexOf(beatmap_property.beatmapset_id) != -1) {
            beatmap.beatmapset_id = this.buff.getInt();
        } else {
            this.buff.skipInt();
        }
        if (this.parse_settings.indexOf(beatmap_property.thread_id) != -1) {
            beatmap.thread_id = this.buff.getInt();
        } else {
            this.buff.skipInt();
        }

        if (this.parse_settings.indexOf(beatmap_property.grade_achieved_std) != -1) {
            beatmap.grade_achieved_std = this.buff.getByte();
        } else {
            this.buff.skipByte();
        }
        if (this.parse_settings.indexOf(beatmap_property.grade_achieved_taiko) != -1) {
            beatmap.grade_achieved_taiko = this.buff.getByte();
        } else {
            this.buff.skipByte();
        }
        if (this.parse_settings.indexOf(beatmap_property.grade_achieved_ctb) != -1) {
            beatmap.grade_achieved_ctb = this.buff.getByte();
        } else {
            this.buff.skipByte();
        }
        if (this.parse_settings.indexOf(beatmap_property.grade_achieved_mania) != -1) {
            beatmap.grade_achieved_mania = this.buff.getByte();
        } else {
            this.buff.skipByte();
        }

        if (this.parse_settings.indexOf(beatmap_property.local_offset) != -1) {
            beatmap.local_offset = this.buff.getShort();
        } else {
            this.buff.skipShort();
        }

        if (this.parse_settings.indexOf(beatmap_property.stack_laniecy) != -1) {
            beatmap.stack_laniecy = this.buff.getSingle();
        } else {
            this.buff.skipSingle();
        }

        if (this.parse_settings.indexOf(beatmap_property.gamemode) != -1) {
        ...
        if (this.parse_settings.indexOf(beatmap_property.source) != -1) {
            beatmap.source = this.buff.getString();
        } else {
            this.buff.skipString();
        }
        if (this.parse_settings.indexOf(beatmap_property.tags) != -1) {
            beatmap.tags = this.buff.getString();
        } else {
            this.buff.skipString();
        }

        if (this.parse_settings.indexOf(beatmap_property.online_offset) != -1) {
            beatmap.online_offset = this.buff.getShort();
        } else {
            this.buff.skipShort();
        }

        if (this.parse_settings.indexOf(beatmap_property.font_title) != -1) {
            beatmap.font_title = this.buff.getString();
        } else {
            this.buff.skipString();
        }

        if (this.parse_settings.indexOf(beatmap_property.is_unplayed) != -1) {
            beatmap.is_unplayed = this.buff.getBool();
        } else {
            this.buff.skipBool();
        }

        if (this.parse_settings.indexOf(beatmap_property.last_played) != -1) {
            beatmap.last_played = this.buff.getDateTime();
        } else {
            this.buff.skipDateTime();
        }

        if (this.parse_settings.indexOf(beatmap_property.is_OSZ2) != -1) {
            beatmap.is_OSZ2 = this.buff.getBool();
        } else {
            this.buff.skipBool();
        }

        if (this.parse_settings.indexOf(beatmap_property.folder_name) != -1) {
            beatmap.folder_name = this.buff.getString();
        } else {
            this.buff.skipString();
        }

        if (this.parse_settings.indexOf(beatmap_property.last_checked_repository_time) != -1) {
            beatmap.last_checked_repository_time = this.buff.getDateTime();
        } else {
            this.buff.skipDateTime();
        }

        if (this.parse_settings.indexOf(beatmap_property.is_ignore_hit_sounds) != -1) {
            beatmap.is_ignore_hit_sounds = this.buff.getBool();
        } else {
            this.buff.skipBool();
        }
        if (this.parse_settings.indexOf(beatmap_property.is_ignore_skin) != -1) {
            beatmap.is_ignore_skin = this.buff.getBool();
        } else {
            this.buff.skipBool();
        }
        if (this.parse_settings.indexOf(beatmap_property.is_disable_storyboard) != -1) {
            beatmap.is_disable_storyboard = this.buff.getBool();
        } else {
            this.buff.skipBool();
        }
        if (this.parse_settings.indexOf(beatmap_property.is_disable_video) != -1) {
            beatmap.is_disable_video = this.buff.getBool();
        } else {
            this.buff.skipBool();
        }
        if (this.parse_settings.indexOf(beatmap_property.is_visual_override) != -1) {
        ...
        if (this.parse_settings.indexOf(beatmap_property.mod_time) != -1) {
            beatmap.mod_time = this.buff.getInt();
        } else {
            this.buff.skipInt();
        }
        if (this.parse_settings.indexOf(beatmap_property.mania_scroll) != -1) {
